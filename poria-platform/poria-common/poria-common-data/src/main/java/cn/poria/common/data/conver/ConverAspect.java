	...
	private void converOneField(Object result, ConverData converData){
		...
		if(!CollectionUtils.isEmpty(keys)){
			...
			if(StrUtil.isBlank(converData.serviceId())){
				...
				List<ConverResult> list = converDao.convertData(keys, converData.table(),dbKey,dbVaule,converData.condition());
				...
				//调用远程接口查询数据
				ConverReq req = new ConverReq(dbKey,dbVaule, converData.table(),keys,converData.condition());
	...
	private void convertValue(Map<String,String> map, Object obj, ConverData converData){
		...
		if(map.get(key) != null){
			...
			if(valueExpression != null){
				if (ObjectUtil.equals(converData.table(),"cus_user_patient")&&ObjectUtil.equals(converData.dbValue(),"idcard")){
					valueExpression.setValue(obj, EncryptTypeUtil.getResultDecrypt(map.get(key)));
				}else {
