...
@Slf4j
@Component
public class PlatAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		if (MapUtil.isNotEmpty(map)) {
			PlatUser userInfo = (PlatUser) map.get(SecurityConstants.DETAILS_USER);
			log.info("用户：{} 登录成功", userInfo.getName());
			SecurityContextHolder.getContext().setAuthentication(accessTokenAuthentication);
			JSONObject jsonObject = JSONUtil.parseObj(userInfo).putOnce("client", ((OAuth2AccessTokenAuthenticationToken) authentication).getRegisteredClient().getClientId());
//			KafkaUtil.send(PlatAuthConstant.AUTH_LOGIN_LOG_TOPIC, JSONUtil.toJsonStr(jsonObject));
		}
		// 输出token
