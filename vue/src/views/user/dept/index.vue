...
<script setup>
  ...
  function resetForm(row = {}) {
    ...
    if (row.deptId !== undefined) form.deptId = row.deptId
  }
  async function load() {
    loading.value = true;
    try {
      rows.value = (await pageApi.deptTree({
        ...query
      })).data || []
    } catch (_) {
      rows.value = []
  ...
  async function submit() {
    saving.value = true;
    try {
      await (form.deptId === undefined ? pageApi.saveDept({
        ...form
      }) : pageApi.updateDept({
        ...form
      }));
  ...
  async function remove(row) {
    if (row.deptId === undefined) return ElMessage.warning('缺少部门标识');
    await ElMessageBox.confirm('确认删除该部门吗？', '系统提示', {
      type: 'warning'
    });
    await pageApi.deleteDept(row.deptId);
