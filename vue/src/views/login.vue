...
async function refreshCaptcha() {
  ...
  try {
    const captcha = await getCodeImg(loginForm.value.randomStr)
...
function handleLogin() {
  loginRef.value.validate(valid => {
    ...
    userStore.login(loginForm.value)
      ...
      .catch(error => {
        ElMessage.error(error?.message || '登录失败，请检查认证服务配置')
        refreshCaptcha()
      })
      .finally(() => {
        loading.value = false
...
.login {
  ...
  display: grid;
  place-items: center;
  background: radial-gradient(circle at 15% 15%, #1d5fae, transparent 38%), #001529;
}
.login-form {
...
.brand span { display: grid; width: 32px; height: 32px; place-items: center; border-radius: 8px; background: #1677ff; color: #fff; font-size: 19px; font-weight: 800; }
