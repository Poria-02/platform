...
function menuId(menu) { return menu.id ?? menu.menuId }
function menuEntries(items = []) { return (Array.isArray(items) ? items : []).flatMap(item => [item, ...menuEntries(item.children || [])]) }
...
function menuPageRoutes(menus) {
  ...
  return menuEntries(menus).flatMap(menu => {
    ...
    const meta = { title: menu.name || menu.label || '未命名菜单', icon: menu.icon || 'menu', sourcePath: menu.path }
    return [{
      path: menu.path, component: Layout, name: `PoriaMenuPage${id}`,
      ...(redirect ? { redirect } : { children: [{ path: '', component: componentFor(menu.path), name: `PoriaMenuView${id}`, meta }] })
    }]
  })
}
...
function menuSidebarRoutes(items = []) {
  return (Array.isArray(items) ? items : [])
    ...
    .map(item => {
      ...
      return {
        path: item.path || `/menu-group/${id}`, name: `PoriaMenu${id}`, alwaysShow: children.length > 0,
        meta: { title: item.name || item.label || '未命名菜单', icon: item.icon || 'menu' },
...
const usePermissionStore = defineStore('permission', {
  ...
  actions: {
    ...
    setDefaultRoutes(routes) { this.defaultRoutes = constantRoutes.concat(routes) },
    setTopbarRoutes(routes) { this.topbarRouters = routes },
    ...
    generateRoutes() {
      ...
      this.setRoutes(routes); this.setSidebarRouters(sidebarRoutes); this.setDefaultRoutes(routes); this.setTopbarRoutes(routes)
